...
import org.contextmapper.generated.gateway.service.CategoryIdService;
...
/**
 * REST controller for managing {@link org.contextmapper.generated.gateway.domain.CategoryId}.
 */
@RestController
@RequestMapping("/api")
public class CategoryIdResource {
    ...
    public Mono<List<CategoryIdDTO>> getAllCategoryIds() {
        log.debug("REST request to get all CategoryIds");
        return categoryIdService.findAll().collectList();
    }

    /**
     * {@code GET  /category-ids} : get all the categoryIds as a stream.
     * @return the {@link Flux} of categoryIds.
     */
    @GetMapping(value = "/category-ids", produces = MediaType.APPLICATION_NDJSON_VALUE)
    public Flux<CategoryIdDTO> getAllCategoryIdsAsStream() {
        log.debug("REST request to get all CategoryIds as a stream");
    ...
    public Mono<ResponseEntity<CategoryIdDTO>> getCategoryId(@PathVariable Long id) {
        log.debug("REST request to get CategoryId : {}", id);
        Mono<CategoryIdDTO> categoryIdDTO = categoryIdService.findOne(id);
