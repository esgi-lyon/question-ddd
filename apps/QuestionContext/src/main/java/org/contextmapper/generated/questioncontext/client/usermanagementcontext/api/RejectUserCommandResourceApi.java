/**
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech) (6.6.0).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
package org.contextmapper.generated.questioncontext.client.usermanagementcontext.api;

import org.contextmapper.generated.questioncontext.client.usermanagementcontext.model.RejectUserCommandDTO;
import io.swagger.v3.oas.annotations.ExternalDocumentation;
import io.swagger.v3.oas.annotations.Operation;
import io.swagger.v3.oas.annotations.Parameter;
import io.swagger.v3.oas.annotations.Parameters;
import io.swagger.v3.oas.annotations.media.ArraySchema;
import io.swagger.v3.oas.annotations.media.Content;
import io.swagger.v3.oas.annotations.media.Schema;
import io.swagger.v3.oas.annotations.responses.ApiResponse;
import io.swagger.v3.oas.annotations.security.SecurityRequirement;
import io.swagger.v3.oas.annotations.tags.Tag;
import io.swagger.v3.oas.annotations.enums.ParameterIn;
import org.springframework.http.HttpStatus;
import org.springframework.http.MediaType;
import org.springframework.http.ResponseEntity;
import org.springframework.validation.annotation.Validated;
import org.springframework.web.bind.annotation.*;
import org.springframework.web.context.request.NativeWebRequest;
import org.springframework.web.multipart.MultipartFile;

import javax.validation.Valid;
import javax.validation.constraints.*;
import java.util.List;
import java.util.Map;
import java.util.Optional;
import javax.annotation.Generated;

@Generated(value = "org.openapitools.codegen.languages.SpringCodegen", date = "2023-05-27T14:01:12.144212+02:00[Europe/Paris]")
@Validated
@Tag(name = "reject-user-command-resource", description = "the reject-user-command-resource API")
public interface RejectUserCommandResourceApi {

    /**
     * POST /api/reject-user-commands
     *
     * @param rejectUserCommandDTO  (required)
     * @return OK (status code 200)
     */
    @Operation(
        operationId = "createRejectUserCommand",
        tags = { "reject-user-command-resource" },
        responses = {
            @ApiResponse(responseCode = "200", description = "OK", content = {
                @Content(mediaType = "*/*", schema = @Schema(implementation = RejectUserCommandDTO.class))
            })
        }
    )
    @RequestMapping(
        method = RequestMethod.POST,
        value = "/api/reject-user-commands",
        produces = "*/*",
        consumes = "application/json"
    )
    ResponseEntity<RejectUserCommandDTO> createRejectUserCommand(
        @Parameter(name = "RejectUserCommandDTO", description = "", required = true) @Valid @RequestBody RejectUserCommandDTO rejectUserCommandDTO
    );


    /**
     * DELETE /api/reject-user-commands/{id}
     *
     * @param id  (required)
     * @return OK (status code 200)
     */
    @Operation(
        operationId = "deleteRejectUserCommand",
        tags = { "reject-user-command-resource" },
        responses = {
            @ApiResponse(responseCode = "200", description = "OK")
        }
    )
    @RequestMapping(
        method = RequestMethod.DELETE,
        value = "/api/reject-user-commands/{id}"
    )
    ResponseEntity<Void> deleteRejectUserCommand(
        @Parameter(name = "id", description = "", required = true, in = ParameterIn.PATH) @PathVariable("id") Long id
    );


    /**
     * GET /api/reject-user-commands
     *
     * @return OK (status code 200)
     */
    @Operation(
        operationId = "getAllRejectUserCommands",
        tags = { "reject-user-command-resource" },
        responses = {
            @ApiResponse(responseCode = "200", description = "OK", content = {
                @Content(mediaType = "*/*", array = @ArraySchema(schema = @Schema(implementation = RejectUserCommandDTO.class)))
            })
        }
    )
    @RequestMapping(
        method = RequestMethod.GET,
        value = "/api/reject-user-commands",
        produces = "*/*"
    )
    ResponseEntity<List<RejectUserCommandDTO>> getAllRejectUserCommands(
        
    );


    /**
     * GET /api/reject-user-commands/{id}
     *
     * @param id  (required)
     * @return OK (status code 200)
     */
    @Operation(
        operationId = "getRejectUserCommand",
        tags = { "reject-user-command-resource" },
        responses = {
            @ApiResponse(responseCode = "200", description = "OK", content = {
                @Content(mediaType = "*/*", schema = @Schema(implementation = RejectUserCommandDTO.class))
            })
        }
    )
    @RequestMapping(
        method = RequestMethod.GET,
        value = "/api/reject-user-commands/{id}",
        produces = "*/*"
    )
    ResponseEntity<RejectUserCommandDTO> getRejectUserCommand(
        @Parameter(name = "id", description = "", required = true, in = ParameterIn.PATH) @PathVariable("id") Long id
    );


    /**
     * PATCH /api/reject-user-commands/{id}
     *
     * @param id  (required)
     * @param rejectUserCommandDTO  (required)
     * @return OK (status code 200)
     */
    @Operation(
        operationId = "partialUpdateRejectUserCommand",
        tags = { "reject-user-command-resource" },
        responses = {
            @ApiResponse(responseCode = "200", description = "OK", content = {
                @Content(mediaType = "*/*", schema = @Schema(implementation = RejectUserCommandDTO.class))
            })
        }
    )
    @RequestMapping(
        method = RequestMethod.PATCH,
        value = "/api/reject-user-commands/{id}",
        produces = "*/*",
        consumes = "application/json"
    )
    ResponseEntity<RejectUserCommandDTO> partialUpdateRejectUserCommand(
        @Parameter(name = "id", description = "", required = true, in = ParameterIn.PATH) @PathVariable("id") Long id,
        @Parameter(name = "RejectUserCommandDTO", description = "", required = true) @Valid @RequestBody RejectUserCommandDTO rejectUserCommandDTO
    );


    /**
     * PUT /api/reject-user-commands/{id}
     *
     * @param id  (required)
     * @param rejectUserCommandDTO  (required)
     * @return OK (status code 200)
     */
    @Operation(
        operationId = "updateRejectUserCommand",
        tags = { "reject-user-command-resource" },
        responses = {
            @ApiResponse(responseCode = "200", description = "OK", content = {
                @Content(mediaType = "*/*", schema = @Schema(implementation = RejectUserCommandDTO.class))
            })
        }
    )
    @RequestMapping(
        method = RequestMethod.PUT,
        value = "/api/reject-user-commands/{id}",
        produces = "*/*",
        consumes = "application/json"
    )
    ResponseEntity<RejectUserCommandDTO> updateRejectUserCommand(
        @Parameter(name = "id", description = "", required = true, in = ParameterIn.PATH) @PathVariable("id") Long id,
        @Parameter(name = "RejectUserCommandDTO", description = "", required = true) @Valid @RequestBody RejectUserCommandDTO rejectUserCommandDTO
    );

}
